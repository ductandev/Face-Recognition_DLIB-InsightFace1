...
while True:
    ...
    if frame is not None:
        ...
        for i in range(boxes.shape[0]):
            ...
            # print((shape[37]+shape[38])/2)
            shape = np.array([shape[30], (shape[37]+shape[38]+shape[40]+shape[41])/4, (shape[43]+shape[44]+shape[47]+shape[46])/4, shape[48], shape[54]])
            print(shape)
            # landmarks = five_point.reshape(1,10)
            # print(shape.T)
            for (x, y) in shape:
                cv2.circle(frame, (int(x), int(y)), 2, (80,18,236), -1)
